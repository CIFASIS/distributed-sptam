...
void StereoMatcher::match(const cv::DescriptorMatcher& matcher, double matchingDistanceThreshold,
                                 const vector<cv::KeyPoint>& kps1, const cv::Mat& descriptors1, const vector<cv::KeyPoint>& kps2, const cv::Mat& descriptors2,
                                 const vector<cv::KeyPoint>& kps3, const cv::Mat& descriptors3, const vector<cv::KeyPoint>& kps4, const cv::Mat& descriptors4,
                                 std::vector<SDMatch>& matches)
{
  // Matching.. use NORM_HAMMING with binary descriptors, NORM_L2 with SIFT and SURF
  RowMatcher row_matcher(25, cv::NORM_HAMMING);

  vector<cv::DMatch> matches12;

  row_matcher.match( kps1, descriptors1,
                 kps2, descriptors2,
                 matches12);

  vector<cv::DMatch> matches34;

  row_matcher.match( kps3, descriptors3,
                 kps4, descriptors4,
                 matches34);

  StereoMatcher::match( matcher, matchingDistanceThreshold,
                     descriptors1, descriptors2,
                     matches12,
                     descriptors3, descriptors4,
                     matches34,
                     matches);
}
...
/*void drawStereoMatches(const sptam::Map::KeyFrame& stereo_frame1, const sptam::Map::KeyFrame& stereo_frame2,
                       const std::vector<SDMatch>& stereo_matches, cv::Mat& out)
{

  cv::Mat img1 = cv::Mat::zeros(stereo_frame1.GetCameraLeft().GetCalibration().imageHeight, stereo_frame1.GetCameraLeft().GetCalibration().imageWidth, CV_32F);
  cv::Mat img2 = cv::Mat::zeros(stereo_frame1.GetCameraRight().GetCalibration().imageHeight, stereo_frame1.GetCameraRight().GetCalibration().imageWidth, CV_32F);
  cv::Mat img3 = cv::Mat::zeros(stereo_frame2.GetCameraLeft().GetCalibration().imageHeight, stereo_frame2.GetCameraLeft().GetCalibration().imageWidth, CV_32F);
  cv::Mat img4 = cv::Mat::zeros(stereo_frame2.GetCameraRight().GetCalibration().imageHeight, stereo_frame2.GetCameraRight().GetCalibration().imageWidth, CV_32F);

  // they should be the same size and type
  cv::Mat img1 = stereo_frame1.GetFrameLeft().image_, img2 = stereo_frame1.GetFrameRight().image_,
          img3 = stereo_frame2.GetFrameLeft().image_, img4 = stereo_frame2.GetFrameRight().image_;
