...
size_t PoseEstimator::estimatePose(const sptam::Map::KeyFrame& referenceFrame, const sptam::Map::KeyFrame& targetFrame, const std::vector<SDMatch>& stereo_matches, Matx44d& pose)
{
  switch(pe_type) {
    case PETYPE::CENTRAL: return centralPoseEstimation(referenceFrame, targetFrame, stereo_matches, pose);
    //case PETYPE::NONCENTRAL: return noncentralPoseEstimation(referenceFrame, targetFrame, stereo_matches);
    default: return centralPoseEstimation(referenceFrame, targetFrame, stereo_matches, pose);
  }
}

size_t PoseEstimator::centralPoseEstimation(const sptam::Map::KeyFrame& referenceFrame, const sptam::Map::KeyFrame& targetFrame, const std::vector<SDMatch>& stereo_matches, Matx44d& pose)
{
  const vector<KeyPoint>& kps3 = targetFrame.GetFrameLeft().GetFeatures().GetKeypoints();

  points_t point_cloud = triangulateReferencePoints(referenceFrame, stereo_matches);

  bearingVectors_t bearingVectors3;

  Matx33d left_calibration = targetFrame.GetCameraLeft().GetCalibration().intrinsic;
...
points_t PoseEstimator::triangulateReferencePoints(const sptam::Map::KeyFrame& referenceFrame, const std::vector<SDMatch>& stereo_matches)
{
  const vector<KeyPoint>& kps1 = referenceFrame.GetFrameLeft().GetFeatures().GetKeypoints();
  const vector<KeyPoint>& kps2 = referenceFrame.GetFrameRight().GetFeatures().GetKeypoints();

  Matx33d left_calibration = referenceFrame.GetCameraLeft().GetCalibration().intrinsic;
  ...
  Matx33d right_calibration = referenceFrame.GetCameraRight().GetCalibration().intrinsic;
  ...
  translation_t leftToright_position = referenceFrame.GetCameraLeft().GetPose().GetOrientationMatrix().transpose() * (referenceFrame.GetCameraRight().GetPosition() - referenceFrame.GetCameraLeft().GetPosition());

  /* left_rotation * right_orientation = T_lw * T_wr = T_lr */
  rotation_t leftToright_orientation = referenceFrame.GetCameraLeft().GetPose().GetOrientationMatrix().transpose() * referenceFrame.GetCameraLeft().GetPose().GetOrientationMatrix();
