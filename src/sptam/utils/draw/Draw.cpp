...
void drawFeatures(const sptam::Map::KeyFrame& frame, cv::Mat& image)
{
  ...
  for( const auto& measurement : frame.measurements() )
  {
    if (measurement->GetType() != Measurement::RIGHT)
    {
      ...
      const std::vector<double> &featurePos_v = measurement->GetProjection();
      cv::Point2d featurePos(featurePos_v[0], featurePos_v[1]);
 ...
 */
void drawProjectionCovariances(const Frame& frame, cv::Mat& image, const ConstIterable<sptam::Map::Point>& points)
{
  ...
  for ( const sptam::Map::Point& mapPoint : points )
  {
    ...
    Ellipse2D ellipse = computeCovarianceEllipse(projection_covariance, PROB_99);
...
/**
 * make a new image merging two stereo frames, copying the images, and return references to each sub-frame.
 */
cv::Mat makeStereoWindow(const cv::Mat& imgL, const cv::Mat& imgR, cv::Mat& outImageLeft, cv::Mat& outImageRight, bool horizontal)
{
  ...
  cv::Mat outImage(height, width, imgL.type());
                  ...
                  const cv::Scalar& color_proj, const cv::Scalar& color_feat, cv::Mat& image)
{
  ...
  {
    const std::vector<double>& featurePos_v = match.measurement.GetProjection();
    cv::Point2d featurePos(featurePos_v[0], featurePos_v[1]);
...
cv::Mat drawMeasuredFeatures(const StereoFrame& frame, const std::list<Match>& measurements, cv::Mat& imgL, cv::Mat& imgR)
{
  ...
  for ( const auto& match : measurements )
  {
    ...
    {
      // store stereo meas as left and right meas
      cv::Point2d proj_left( match.measurement.GetProjection()[0], match.measurement.GetProjection()[1] );
      cv::Point2d proj_right( match.measurement.GetProjection()[2], match.measurement.GetProjection()[1] );

      meas_left.push_back( Match(match.mapPoint, Measurement(Measurement::LEFT, match.measurement.GetSource(), proj_left, match.mapPoint->GetDescriptor())) );
      meas_right.push_back( Match(match.mapPoint, Measurement(Measurement::RIGHT, match.measurement.GetSource(), proj_right, match.mapPoint->GetDescriptor())) );
    }
  }

  // get projections matrices
  cv::Matx34d projetion_left = frame.GetFrameLeft().GetProjection();
  cv::Matx34d projetion_right = frame.GetFrameRight().GetProjection();

  // draw measurements
//  outImageLeft = cv::Mat(outImage, cv::Rect(0, 0, imgL.cols, imgL.rows));
//  imgL.copyTo( outImageLeft );
  drawMeasurements(projetion_left, meas_left, COLOR_YELLOW, COLOR_RED, imgL);

//  outImageRight = cv::Mat(outImage, cv::Rect(0, imgL.rows, imgR.cols, imgR.rows));
//  imgR.copyTo( outImageRight );
  drawMeasurements(projetion_right, meas_right, COLOR_YELLOW, COLOR_RED, imgR);
