...
void BundleDriver::SetData(ConstIterable<sptam::Map::SharedKeyFrame>& adjustViews, ConstIterable<sptam::Map::SharedKeyFrame>& fixedViews)
{
  ...
  {
    G2ODriver::Vertex* camera_vertex = g2o_driver_.AddVertex( keyFrame->GetCameraPose(), false, new CameraVertexData( keyFrame ) );
  ...
  {
    G2ODriver::Vertex* camera_vertex = g2o_driver_.AddVertex( keyFrame->GetCameraPose(), true, new CameraVertexData( keyFrame ) );
    view_to_vertex[ keyFrame.get() ] = camera_vertex;
