...
inline bool compareX(const Match& i, const Match& j)
{
  ...
  const std::vector<double>& i_projection = i_meas.GetProjection();
  const std::vector<double>& j_projection = j_meas.GetProjection();

  return i_projection[0] < j_projection[0];
}
...
inline int getMinX(const std::list<Match>& measurements)
{
  ...
  return min.measurement.GetProjection()[0];
}
