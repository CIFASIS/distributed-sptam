...
PointCloud::PointCloud(CameraPose& cameraPoseTracked, const sptam::Map& map,
  ...
  : cameraPoseTracked_(cameraPoseTracked), map_(map), thread_(&PointCloud::operator(), this)
{
...
void PointCloud::operator()(){
  ...
  while (!stop_ || viewer_->wasStopped()) {
    ...
    if( updated ) {
      ...
      FrustumCulling frustumCulling(cameraPoseTracked_, horizontalFOV_,  verticalFOV_, nearPlaneDist_, farPlaneDist_);
...
pcl::PolygonMesh visualizerConcatenatePolygonmeshes(GenericContainer& meshes) {
  ...
  while (current != end)
  {
    ...
    for(int i=0;i<current_poly.polygons.size();i++) {
...
void addCameraPoses(const std::vector<StereoFrame*>& keyFrames, boost::shared_ptr<pcl::visualization::PCLVisualizer> viewer)
{
  for(uint i = 0 ; i < keyFrames.size(); ++i) {
    const CameraPose& cameraPose = keyFrames[i]->GetCameraLeft().GetPose();
                                                                           ...
                                                                           const double farPlaneDist = 0)
{
  ...
  FrustumCulling frustumCulling(CameraPose(), horizontalFOV, verticalFOV, nearPlaneDist, farPlaneDist);
