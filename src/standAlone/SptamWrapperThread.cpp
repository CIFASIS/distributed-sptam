...
SptamWrapperThread::SptamWrapperThread(
  ...
  , stop_( false )
  , thread_(&SptamWrapperThread::run, this)
{}
...
void SptamWrapperThread::run() {
  ...
  while( not stop_ ) {
    ...
    if (not isMapInitialized_)
    {
      ...
      // Initialize MapMaker
      isMapInitialized_ = sptam_.initFromStereo(currentCameraPose, imageFeaturesLeft, imageFeaturesRight);
    }
     ...
     */
    #ifdef SHOW_PROFILING
      WriteToLog("BASE_LINK_POSE:", currentFrameIndex_, currentCameraPose.GetPosition(), currentCameraPose.GetOrientationMatrix());
    #endif
